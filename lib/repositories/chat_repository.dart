...
import 'package:cuidapet_api/models/chat_fornecedor_model.dart';
...
class ChatRepository {
  ...
  Future<List<ChatFornecedorModel>> buscarChatsFornecedor(
      int fornecedor) async {
    MySqlConnection conn;
    try {
      conn = await DatabaseConnection.openConnection();
      final result = await conn.query(''' 
        select c.id, c.data_criacao, c.status, a.nome, a.nome_pet, a.fornecedor_id, f.nome, f.logo, a.usuario_id, u.img_avatar, u.tipo_cadastro, u.email
        from chats as c
          inner join agendamento a on c.agendamento_id = a.id
          inner join fornecedor f on a.fornecedor_id = f.id
      ...
      return result.map((e) {
        return ChatFornecedorModel(
          ...
          usuario: UsuarioModel(
              id: e[8] as int,
              imgAvatar: (e[9] as Blob).toString(),
              tipoCadastro: e[10] as String,
              email: e[11] as String),
        );
      }).toList();
    } catch (e) {
      print(e);
