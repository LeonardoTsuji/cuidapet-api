...
class ChatController extends ResourceController {
  ...
  Future<Response> buscarChatsFornecedor(UsuarioModel user) async {
    final chats = await _chatRepository.buscarChatsFornecedor(user.id);
    return Response.ok(chats
        .map((a) => {
              'id': a.id,
              'usuario': {
                'id': a.usuario.id,
                'imgAvatar': a.usuario.imgAvatar,
                'tipoCadastro': a.usuario.tipoCadastro,
